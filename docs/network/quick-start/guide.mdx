...
# Quick Start Guide to the Log Store Network
...
The CLI tool, suitable for command-line interactions, can be installed with `npm i -g @logsn/cli`.
For programmatic access, the JavaScript Client/SDK is available via `npm i @logsn/client`.
