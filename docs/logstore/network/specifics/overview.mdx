...
### Streamr Storage Provider

The Log Store Network is natively embedded into the Streamr Network as a Storage Provider.

This means that streams created via the [Streamr Hub](https://streamr.network/hub/streams) or SDKs can be allocated to the Log Store Network for storage without using the Log Store SDK.

Learn more about [using the Streamr SDK](../quick-start/using-streamr-sdk.md).
